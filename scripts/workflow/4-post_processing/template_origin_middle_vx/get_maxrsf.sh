...
do
  ii=$(( ${inn} * 5 + 50 ))
  dataname=${datapath}/model_${mnum}_shot_${ii}_vx.bin
  echo "in=${dataname} n1=126 n2=12004 data_format=native_float" > model_${mnum}_shot_${ii}_vx_raw.rsf
  sfmath < model_${mnum}_shot_${ii}_vx_raw.rsf output="abs(input)" |\
    sfmax axis=2 max=y | \
    sfput d2=0.02 d1=20 o1=500 o2=0 > model_${mnum}_shot_${ii}_vx_max.rsf
done
