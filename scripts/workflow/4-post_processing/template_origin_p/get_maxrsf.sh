...
do
  ii=$(( ${inn} * 5 + 50 ))
  dataname=${datapath}/model_${mnum}_shot_${ii}_div.bin
  echo "in=${dataname} n1=126 n2=12004 data_format=native_float" > model_${mnum}_shot_${ii}_div_raw.rsf
  sfmath < model_${mnum}_shot_${ii}_div_raw.rsf output="abs(input)" |\
    sfmax axis=2 max=y > model_${mnum}_shot_${ii}_div_max.rsf
done
