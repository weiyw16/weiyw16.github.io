...
do
  ii=$((${inn} * 5 + 50 ))
  dataname=${datapath}/model_${mnum}_shot_${ii}_curl.bin
  echo "in=${dataname} n1=126 n2=12004 data_format=native_float" > model_${mnum}_shot_${ii}_curl_raw.rsf
  sfmath < model_${mnum}_shot_${ii}_curl_raw.rsf output="abs(input)" |\
    sfmax axis=2 max=y > model_${mnum}_shot_${ii}_curl_max.rsf
done
