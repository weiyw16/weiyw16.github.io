...
// parameters
  ...
  int snap_step;
	int is, ir, irx, irz, it, ix, iz, nabs, fnabs, pos, posrec;
	int nx, nz, NX, NZ, NK, NS, NZR, NXR, NT;
	int sz, sx0, sdx, rz0, rdz, rx0, rdx;
	int *src_loc, *rec_loc, *this_src_loc;
	float dx, dz, dt, freq0, amp0;
	float Vmin, Vmax, wave;
	float *record_vz, *record_vx, *div, *curl;
  float *snap_vz, *snap_vx, *snap_div, *snap_curl;
	float *C, *txx, *tzz, *txz, *vx, *vz, *buf_array;// readin C
  ...
  char sourcefile[25], receiverfile[25];
  char snapf_vz[126], snapf_vx[126], snapf_div[126], snapf_curl[126];
...
// readin parameters
	if( !(pafid=fopen(paramfile,"r")) ) {
		...
		fscanf(pafid,"%i %i %i %i %i %i", &rx0, &rdx, &NXR, &rz0, &rdz, &NZR);
...
// update parameters and initial matrix
  ...
  fprintf(fid, "rx0 %i rdx %i NXR %i, rz0 %i rdz %i NZR %i\n", rx0, rdx, NXR, rz0, rdz, NZR);
...
// init all the global matrix
  ...
  pml_para = init_matrix( nabs*12 );// 6 * 2
	...
	rec_loc = init_matrix_int( 2 * NZR * NXR);
	this_src_loc = init_matrix_int( 2 );
  buf_array = init_matrix( NZR * NT );
	record_vz = init_matrix( NZR * NXR * NT );
	record_vx = init_matrix( NZR * NXR * NT );
	div = init_matrix( NZR * NXR * NT );
	curl = init_matrix( NZR * NXR * NT );
	snap_vz = init_matrix( nx * nz );
	snap_vx = init_matrix( nx * nz );
	snap_div = init_matrix( nx * nz );
	snap_curl = init_matrix( nx * nz );
  ...
  if (! readin_receiver){
    for(irx = 0;  irx < NXR; irx++)
  	 for(irz = 0; irz < NZR; irz++){
  	    rec_loc[irx * NZR * 2 + irz * 2 + 0] = rx0 + rdx * irx;
  	    rec_loc[irx * NZR * 2 + irz * 2 + 1] = rz0 + rdz * irz;
  	}
  }
  else{
    sprintf(receiverfile, "receiverfile.txt");
    ...
    fprintf(fid,"Error opening param file %s\n", receiverfile);
     ...
     readin_coor(srfid, NZR*NXR, rec_loc, dx, dz);
    }
  }
  ...
  fprintf(fid,"\n\n receiver position \n\n");
  for(ix=0; ix<NZR; ix++){fprintf(fid, "%i %i\n", rec_loc[ix*2+0], rec_loc[ix*2+1]);}
...
// check
  ...
  int *dev_rec_loc = get_deviceMem_int(rec_loc,  NZR*NXR);
...
/*
    ...
    for (irz = 0; irz <  NZR; irz++){
      ir = irx * NZR + irz;
      posrec = irx * NZR * NT + irz + it * NZR;
	  	pos = (rec_loc[ir * 2 + 0] + nabs) * NZ + rec_loc[ir * 2 + 1] + fnabs;
//  		record_vx[it * NR + ir] = vx[pos];
//  		record_vz[it * NR + ir] = vz[pos];
	  	record_vx[posrec] = 0.5 * ( vx[pos] + vx[pos + NZ] );
	  	record_vz[posrec] = 0.5 * ( vz[pos] + vz[pos - 1] );
	  	div[posrec] =  (coe1 * (vx[pos + NZ] - vx[pos])
    ...
    if (save_snap && it % snap_step == 0){
      for (ix=0; ix < nx; ix++)
        for (iz=0; iz < nz; iz++){
          ...
          pos = (ix + nabs) * NZ + iz + fnabs;
	  	    snap_div[ix*nz + iz] =  (coe1 * (vx[pos + NZ] - vx[pos])
	  	    	 			    + coe2 * (vx[pos + 2*NZ] - vx[pos - NZ])) / dx
	  	    	 				+(coe1 * (vz[pos] - vz[pos - 1])
	  	    	 				+ coe2 * (vz[pos + 1] - vz[pos - 2])) / dz;
	  	    snap_curl[ix*nz + iz] = 0.25 * (
	  	    	( (coe1*(vx[pos] - vx[pos-1])+coe2*(vx[pos+1] - vx[pos-2]))/dz) 
	  	    	- ( (coe1*(vz[pos-1] - vz[pos-NZ-1])+coe2*(vz[pos+NZ-1] - vz[pos-2*NZ-1]))/dx)
	  	    	+( (coe1*(vx[pos+1] - vx[pos])+coe2*(vx[pos+2] - vx[pos-1]))/dz) 
	  	    	- ( (coe1*(vz[pos] - vz[pos-NZ])+coe2*(vz[pos+NZ] - vz[pos-2*NZ]))/dx)
  	  ...
  	  sprintf(snapf_div, "snap_output/model_%s_shot_%s_it_%d_div.bin", argv[1], argv[2], it);
  	  if( !(oufid=fopen(snapf_div,"wb")) ) {
		  fprintf(fid,"Error opening output file %s\n", snapf_div);
      	return(1); }
  	  else fwrite(snap_div, sizeof(float), nz*nx, oufid);
  	  fclose(oufid);

  	  sprintf(snapf_curl, "snap_output/model_%s_shot_%s_it_%d_curl.bin", argv[1], argv[2], it);
  	  if( !(oufid=fopen(snapf_curl,"wb")) ) {
		  fprintf(fid,"Error opening output file %s\n", snapf_curl);
      	return(1); }
  	  else fwrite(snap_curl, sizeof(float), nz*nx, oufid);
  	  fclose(oufid);
    ...
    for (irx=0; irx< NXR; irx++){
    	sprintf(outfile_vz, "output/model_%s_shot_%i_vz.bin", argv[1], (atoi(argv[2]) / 50 - 1 ) * NXR + irx);
    	sprintf(outfile_vx, "output/model_%s_shot_%i_vx.bin", argv[1], (atoi(argv[2]) / 50 - 1 ) * NXR + irx);
    	sprintf(outfile_div, "output/model_%s_shot_%i_div.bin", argv[1], (atoi(argv[2]) / 50 - 1 ) * NXR + irx);
    	sprintf(outfile_curl, "output/model_%s_shot_%i_curl.bin", argv[1], (atoi(argv[2]) / 50 - 1 ) * NXR + irx);

      mymemorycp(buf_array, record_vz, irx*NT*NZR, NT*NZR);
    	if( !(oufid=fopen(outfile_vz,"wb")) ) {
  		fprintf(fid,"Error opening output file %s\n", outfile_vz);
      	return(1); }
    	else fwrite(buf_array, sizeof(float), NT*NZR, oufid);
    	fclose(oufid);

      mymemorycp(buf_array, record_vx, irx*NT*NZR, NT*NZR);
      ...
      mymemorycp(buf_array, div, irx*NT*NZR, NT*NZR);
      ...
      mymemorycp(buf_array, curl, irx*NT*NZR, NT*NZR);
